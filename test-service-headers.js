  ...
  try {
    ...
    if (data.success && data.logs && data.logs.length > 0) {
      ...
      if (processedLog.service === 'payments-service') {
        ...
        console.log(
          '❌ FAILED: service.name header was not extracted correctly.',
        );
        console.log(
          `Expected service: 'payments-service', got: '${processedLog.service}'`,
        );
      }
    } else {
  ...
  try {
    ...
    if (data.success && data.logs && data.logs.length > 0) {
      ...
      if (processedLog.service === 'header-service') {
        ...
        console.log(
          `Expected service: 'header-service', got: '${processedLog.service}'`,
        );
      }
    } else {
      console.log('❌ FAILED: No logs were returned in the response.');
