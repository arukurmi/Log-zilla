...
echo "Setting up logzilla command on host system..."
...
# Create the logzilla wrapper script
# Get the current project directory
PROJECT_PATH="$(pwd)"

sudo tee /usr/local/bin/logzilla > /dev/null << EOF
#!/bin/sh
...
if [ \$# -gt 0 ]; then
    ...
    echo "🚀 Running command: \$* (logs will be sent to logzilla)"
    ...
    echo "Usage: logzilla <command>"
    echo "Example: logzilla npm run start:dev"
fi
EOF

sudo chmod +x /usr/local/bin/logzilla

echo ""
echo "✅ Build complete!"
echo ""
echo "🚀 To get started:"
echo "1. Start the logzilla server:"
echo "   npm start"
echo ""
echo "2. Use logzilla in any project directory:"
